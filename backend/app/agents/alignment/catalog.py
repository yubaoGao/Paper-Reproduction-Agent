...
def _merge(base,semantic,field,status_field="status"):
    ...
    strong={"aligned","conflicted","matched","value_conflict","paper_only","code_fallback","scientific_default","conflict"}
...
def _all_evidence(catalog):
    ...
    for group in (catalog.experiment_alignments,catalog.dataset_mappings,catalog.model_mappings,catalog.parameter_mappings,catalog.ablation_mappings,catalog.metric_mappings,catalog.evaluation_policy_alignments):
...
class AlignmentCatalogMerger:
    def merge(self,paper,repository,deterministic,stages,missing=(),warnings=()):
        ...
        evaluation_policies=_merge(deterministic.evaluation_policies,[x for _,s in stages for x in s.evaluation_policy_alignments],"alignment_id")
...
class PaperCodeAlignmentValidator:
    ...
    def validate(self,catalog,paper,repository,*,paper_document=None,repository_snapshot=None,static_analysis=None):
        ...
        mapping_ids={x.alignment_id for x in (*catalog.dataset_mappings,*catalog.model_mappings,*catalog.parameter_mappings,*catalog.ablation_mappings,*catalog.metric_mappings,*catalog.evaluation_policy_alignments)};conflict_ids={x.conflict_id for x in catalog.conflicts}
        ...
        for item in catalog.experiment_alignments:
            ...
            if not set((*item.parameter_mapping_ids,*item.ablation_mapping_ids,*item.metric_mapping_ids))<=mapping_ids or item.dataset_mapping_id and item.dataset_mapping_id not in mapping_ids or item.model_mapping_id and item.model_mapping_id not in mapping_ids or item.evaluation_policy_alignment_id and item.evaluation_policy_alignment_id not in mapping_ids or not set(item.conflict_ids)<=conflict_ids:raise AlignmentValidationError(f"dangling mapping reference: {item.alignment_id}")
        ...
        repository_policy_ids={x.policy_id for x in repository.evaluation_policies}
        for item in catalog.evaluation_policy_alignments:
            if item.paper_experiment_id not in experiments or not set(item.repository_policy_ids)<=repository_policy_ids or item.conflict_id and item.conflict_id not in conflict_ids:raise AlignmentValidationError(f"dangling evaluation policy alignment: {item.alignment_id}")
            self.evidence.validate((*item.paper_evidence,*item.repository_evidence),paper,repository,**backing)
        for conflict in catalog.conflicts:
